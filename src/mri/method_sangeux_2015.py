...
legs = ['R', 'L']
subjects = ['010'] 
scale_factors = [0.55, 0.36, 0.24] # sacle factors if image is distorted [ap, ml, v]
for subject in subjects:
    for side in legs:
        if side == 'R':
            fname = r"C:\Users\Bas\ucloud\MRI_segmentation_BG\markups\\" + subject + "_right.mrk.json"
        elif side == 'L':
            fname = r"C:\Users\Bas\ucloud\MRI_segmentation_BG\markups\\" + subject + "_left.mrk.json"
        else:
        ...
        hjcP = np.array(controlPoints[hjcI]['position']) / scale_factors
        gtP = np.array(controlPoints[gtI]['position']) / scale_factors
        psP = np.array(controlPoints[psI]['position']) / scale_factors
        dsP = np.array(controlPoints[dsI]['position']) / scale_factors
        lcP = np.array(controlPoints[lcI]['position']) / scale_factors
        mcP = np.array(controlPoints[mcI]['position']) / scale_factors
